...
export async function GET() {
  ...
  const { data: properties, error } = await supabase
    .from('properties')
    .select('id, reference_number, title, property_type, availability, price_aed, location_area, community, full_address, latitude, longitude, bedrooms, bathrooms, area_sqft, agent_name')
    .eq('published', true)
