...
export default function UsersPage() {
  ...
  const handleSave = async () => {
    ...
    if (editUser) {
      // Editing existing user — just update, no email
      setUsers(users.map((u) => u.id === editUser.id ? { ...u, ...form } : u));
      setShowModal(false);
      showSaved();
    } else {
      // Creating new user — generate password and send welcome email
      ...
      try {
        ...
        if (error) {
          setEmailStatus({ type: 'error', message: `User created but email failed: ${error.message}` });
        } else {
          setEmailStatus({ type: 'success', message: `Welcome email sent to ${form.email}` });
        }
      } catch (err: any) {
        setEmailStatus({ type: 'error', message: `User created but email failed: ${err.message}` });
      } finally {
        setSendingEmail(false);
      }

      // Add user to list regardless of email status
      setUsers(prev => [...prev, { id: Date.now(), ...form, lastLogin: 'Never' }]);
      showSaved();

      // Keep modal open briefly to show email status, then close
      setTimeout(() => {
        setShowModal(false);
        setEmailStatus(null);
      }, 3000);
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {/* Email Status Banner */}
              {emailStatus && (
                <div className={`flex items-center gap-3 px-4 py-3 border ${emailStatus.type === 'success' ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-red-500/30 bg-red-500/10'}`}>
                  <Icon name={emailStatus.type === 'success' ? 'CheckCircleIcon' : 'ExclamationCircleIcon'} size={16} className={emailStatus.type === 'success' ? 'text-emerald-400' : 'text-red-400'} />
                  <p className={`text-xs font-medium ${emailStatus.type === 'success' ? 'text-emerald-400' : 'text-red-400'}`}>{emailStatus.message}</p>
                </div>
              )}
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-border">
              ...
              <button
                onClick={handleSave}
                disabled={!form.name || !form.email || sendingEmail}
