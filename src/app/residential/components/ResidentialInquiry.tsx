...
const INQUIRY_TYPES = ['Buy', 'Rent', 'Investment', 'Valuation', 'General Enquiry'];
const PROPERTY_TYPES = ['Apartment', 'Villa', 'Townhouse', 'Penthouse', 'Any'];

export default function ResidentialInquiry() {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="animate-on-scroll">
            <span className="text-xs font-bold uppercase tracking-[0.3em] text-primary mb-4 block">Speak With an Advisor</span>
            <h2 className="text-3xl md:text-5xl font-bold text-foreground tracking-tighter leading-none mb-8">
              Find Your<br /><span className="text-gold-shimmer">Ideal Home</span>
            </h2>
            <p className="text-muted-foreground text-base leading-relaxed mb-10 max-w-md">
              Our residential specialists have deep knowledge of Dubai's most sought-after communities. Share your requirements and we'll curate a shortlist tailored to your lifestyle.
            </p>
            <div className="space-y-4">
              {[
                { icon: 'HomeIcon', title: 'Curated Shortlist', desc: 'Receive a personalised selection of properties matching your exact criteria' },
                { icon: 'ShieldCheckIcon', title: 'Off-Market Access', desc: 'Gain access to exclusive listings not available on public portals' },
                { icon: 'UserGroupIcon', title: 'Dedicated Advisor', desc: 'One point of contact throughout your entire search and acquisition' },
              ].map((item) => (
            ...
            {submitted ? (
              <div className="bg-background border border-primary/30 p-12 text-center gold-glow-animate">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed">
                  Thank you for your enquiry. A residential specialist will be in touch within 24 hours.
                </p>
              </div>
            ) : (
              <div className="bg-background border border-border p-8 md:p-10">
                <h3 className="text-xl font-bold text-foreground mb-6 tracking-tight">Submit Your Enquiry</h3>
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {PROPERTY_TYPES.map((pt) => (
                        <button
                          key={pt}
                          type="button"
                          onClick={() => setForm({ ...form, property: pt })}
                          className={`py-2.5 text-xs font-bold border transition-all duration-300 ${form.property === pt ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:border-primary hover:text-foreground'}`}
                        >
                          {pt}
                        </button>
                      ))}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Preferred location, budget, move-in timeline, or any specific requirements..."
                    />
                  </div>
                  <button
                    type="submit"
